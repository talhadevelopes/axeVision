  ...
  private static async buildAccessibilityContext(
    ...
    try {
      ...
      const issues = await prisma.accessibilityIssue.findMany({
        where: { snapshotId: context.snapshotId },
      });

      // Get historical snapshots for this website (last 5)
      const historicalSnapshots = await prisma.snapshot.findMany({
        where: {
      ...
      return {
        ...
        historical: {
          ...
          snapshots: historicalSnapshots.map((s) => ({
            id: s.id,
